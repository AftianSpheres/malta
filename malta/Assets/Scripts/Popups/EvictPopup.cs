...
public class EvictPopup : MonoBehaviour
{
	...
	void Update ()
    {
        if (GameDataManager.Instance != null)
        {
            ...
            if (faeButton.gameObject.activeInHierarchy && !GameDataManager.Instance.HasFlag(ProgressionFlags.FaeUnlock)) faeButton.gameObject.SetActive(false);
            else if (!faeButton.gameObject.activeInHierarchy && GameDataManager.Instance.HasFlag(ProgressionFlags.FaeUnlock)) faeButton.gameObject.SetActive(true);
            if (orcButton.gameObject.activeInHierarchy && !GameDataManager.Instance.HasFlag(ProgressionFlags.OrcUnlock)) orcButton.gameObject.SetActive(false);
            else if (!orcButton.gameObject.activeInHierarchy && GameDataManager.Instance.HasFlag(ProgressionFlags.OrcUnlock)) orcButton.gameObject.SetActive(true);
        }
	}
